...
import { getCallCenterInfo, callCenterPhones } from "@/lib/info";
...
export default async function CallCenterPage({
  ...
  const row = await getCallCenterInfo();

  const phones = callCenterPhones(row);
        ...
        <div className="bg-gray-100 flex items-center">
          <div className="w-full max-w-xl px-6 md:px-12 py-16 md:py-24 me-auto">
            ...
            <div className="mt-8 border-t border-gray-300 pt-8 flex flex-col gap-8">
              {/* Telephone — only when the DB has numbers */}
              {phones.length > 0 ? (
              ...
              {/* Email — only when set in the dashboard */}
              {email ? (
                <div className="flex items-start gap-3">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" className="mt-0.5 text-gray-400 shrink-0">
                  ...
                  <div>
                    <p className="text-sm text-gray-500">{dict.email}</p>
                    <a
                      href={`mailto:${email}`}
                      className="mt-1 block font-bold text-gray-900 break-all"
                      dir="ltr"
                    >
                      {email}
                    </a>
